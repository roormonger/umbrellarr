...
export function ArtistsToolbar({
  ...
  onSortChange: (key: ArtistSortKey) => void;
  onFilterChange: (key: ArtistFilterKey) => void;
  onRefresh: () => void;
  refreshing?: boolean;
}) {
  const sortLabel = ARTIST_SORT_OPTIONS.find((o) => o.value === sortKey)?.label ?? "Sort";
  const filterLabel = ARTIST_FILTER_OPTIONS.find((o) => o.value === filterKey)?.label ?? "Filter";

...
import { grabMovieRelease } from "@/api/movies";
...
type Props = {
  ...
  history: MovieHistoryEvent[];
  blocklist: MovieBlocklistItem[];
};
...
/** Mirror Radarr MovieLanguages: single name, or "Multi-Language" + hover list. */
function LanguagesCell({ languages }: { languages: MovieReleaseLanguage[] }) {
...
function ProtocolBadge({ protocol }: { protocol: MovieRelease["protocol"] }) {
  if (protocol === "usenet") {
    return (
  ...
  const grabMutation = useMutation({
    mutationFn: (override: boolean) => {
      if (override) {
        ...
        return grabMovieRelease(instanceId, {
          guid: release.guid,
      ...
      return grabMovieRelease(instanceId, {
        guid: release.guid,
        indexerId: release.indexerId,
        movieId: release.downloadAllowed ? undefined : movieId,
      });
    },
    onMutate: (override) => {
      setGrabError(null);
    ...
    onSuccess: (_data, override) => {
      ...
      notifications.show({
        color: "green",
        message: override ? "Override grab sent to Radarr" : "Release added to download queue",
      });
    },
...
export function MovieInteractiveSearchTable({
  instanceId,
  movieId,
  releases,
  history,

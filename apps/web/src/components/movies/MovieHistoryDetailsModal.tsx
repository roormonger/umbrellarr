...
import { markMovieHistoryFailed } from "@/api/movies";
import { formatFreeSpace } from "@/lib/moviePath";
import classes from "./MovieHistoryDetailsModal.module.css";

type Props = {
  opened: boolean;
  onClose: () => void;
  instanceId: string;
  event: MovieHistoryEvent | null;
};
...
/** Mirror Radarr HistoryDetailsModal titles (en.json). */
function detailTitle(eventType: MovieHistoryEventType): string {
  switch (eventType) {
    case "grabbed":
      return "Grabbed";
    case "downloadFailed":
...
/** Radarr DeletedReason* strings with appName = Radarr. */
function deletedReasonMessage(reason: string | undefined): string {
  switch (reason) {
    case "Manual":
      return "File was deleted using Radarr, either manually or by another tool through the API";
    case "MissingFromDisk":
      return "Radarr was unable to find the file on disk so the file was unlinked from the movie in the database";
    case "Upgrade":
...
function detailRows(event: MovieHistoryEvent): DetailEntry[] {
  ...
  if (eventType === "grabbed") {
    return [
      ...
      { label: "Movie Match Type", value: data.movieMatchType },
  ...
  if (eventType === "downloadFolderImported" || eventType === "movieFolderImported") {
    return [
      { label: "Name", value: sourceTitle },
...
export function MovieHistoryDetailsModal({ opened, onClose, instanceId, event }: Props) {
  const queryClient = useQueryClient();
  const markFailed = useMutation({
    mutationFn: () => {
      if (!event) throw new Error("No history event");
      return markMovieHistoryFailed(instanceId, event.id);
    },
    onSuccess: async () => {
      notifications.show({ color: "green", message: "Marked as failed" });
      await queryClient.invalidateQueries({ queryKey: ["movie-history", instanceId] });
      onClose();
    },
    onError: (error) => {
      notifications.show({
